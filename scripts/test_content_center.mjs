...
for (const need of ['data-cv-label="0"', 'data-cv-artist="0"', 'data-cv-pub="0"', 'data-cv-del="0"']) {
  if (!rec.includes(need)) fail('the recap row is missing ' + need);
}
if (!fails) pass('recap rows edit label, artist tag and live/staged in place');
...
if (!/cv-staged/.test(rec)) fail('the staged row is not marked as staged');
else pass('staged rows say so');

// ---- the library -------------------------------------------------------------
const lib = render('content library', () => api.hubAssetsHTML());
for (const need of ['data-ca-field="label"', 'data-ca-field="kind"', 'data-ca-field="media"',
...
for (const need of ['data-sp-field="title"', 'data-sp-field="stage"', 'data-sp-field="scheduled_for"']) {
