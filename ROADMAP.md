...
| 3 🟡 | Admin dashboard — read | `dashboard.html` rewritten to read from Supabase (inquiries, agreements, income, expenses, equipment). Berky logs in via magic-link, sees real data. No writes yet — read-only de-risks the schema before exposing edits | 2 |
| 4 | Admin dashboard — write | CRUD on agreements, inquiry status updates, income/expense entry, equipment management. This is where Berky stops using Sheets day-to-day | 3 |
